...
def myNotes(request):
    ...
    try:
        notes = NoteListSerializer(Note.objects.filter(user=user).order_by('updated'), many=True)
...
def readNote(request, noteid):
    ...
    try:
        data = NoteSerializer(Note.objects.get(id=noteid, user=user)).data
...
def updateNote(request, noteid):
    ...
    try:
        ...
        note.content = json.dumps(request.data['content'])
...
def createNoteShareExternal(request, noteid):
    ...
    if user == note_owner.id:
        rid = uuid.uuid4()
        active = request.data['active'] if type(request.data['active']) is bool else True
        title = request.data['title']
...
def readNoteShareExternal(request, nui, permkey):
    try:
        query = ShareExternal.objects.select_related('note','user').get(passkey=hashThis(permkey),note__id__startswith=nui,active=1)
...
def readAllLinksNoteShareExt(request, noteid):
    user = getUserID(request)
    links = ShareExternalSerializer(ShareExternal.objects.filter(note=noteid,user=user).values('id','anonymous','created','title','active').order_by('-created'), many=True).data
